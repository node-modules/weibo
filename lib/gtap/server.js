...
exports.start = function(port, store_dir) {
	...
	var cache = new FileStore(store_dir + '/user_cache');
	...
	var server = connect.createServer(
		...
		connect.session({secret: 'gtap-server-session-secret', store: new FileStore(store_dir + '/.sessions')}),
		...
		function(req, res, next) {
		    ...
		    if (!authorization) return connect.utils.unauthorized(res, realm);
